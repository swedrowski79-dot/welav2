    ...
    switch ($action) {
        ...
        case 'sync_categories_batch':
            ...
            foreach ($items as $item) {
                ...
                try {
                    $data = wela_sync_category_request($pdo, $syncPayload);
                    $results[] = [
                        'queue_id' => $queueId,
                        'entity_id' => $entityId,
...
function wela_require_xt_bootstrap(): void
{
    if (defined('WELA_XT_IMAGE_BOOTSTRAPPED')) {
        return;
    }

    $xtBootstrapPath = __DIR__ . '/bootstrap/xtcommerce.php';
...
function wela_store_document_file(array $welaConfig, string $fileName, string $contentBase64, ?string $targetPath = null, string $imageClass = 'product'): array
{
    ...
    $targetDir = wela_resolve_document_target_directory($welaConfig, $targetPath, true);
    ...
    $bytesWritten = file_put_contents($targetPath, $binary, LOCK_EX);
    if ($bytesWritten === false) {
        $lastError = error_get_last();
        throw new RuntimeException(
            "Dokument konnte nicht nach '{$targetPath}' geschrieben werden."
    ...
    if (!is_file($targetPath)) {
        throw new RuntimeException("Datei wurde laut PHP geschrieben, ist am Ziel aber nicht vorhanden: '{$targetPath}'.");
    }
...
{
    $resolved = wela_resolve_document_target_directory($welaConfig, $path, false);
...
function wela_resolve_document_target_directory(array $welaConfig, ?string $requestedPath = null, bool $createIfMissing = false): string
{
    $candidate = $requestedPath;

    if ($candidate === null || trim($candidate) === '') {
        $welaConfiguredPath = $welaConfig['document_upload_path'] ?? null;
        $candidate = is_string($welaConfiguredPath) && trim($welaConfiguredPath) !== ''
            ? trim($welaConfiguredPath)
