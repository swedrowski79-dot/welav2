...
Implement ticket `T-025` by adding raw import and normalization for AFS document records only, then validate it against the live AFS source.
...
- Validated the live import path end to end: migration `006` is registered, `run_import_products.php` succeeds, and `raw_afs_documents` is populated.
...
- Observed:
  ...
  - `run_import_products.php` completed successfully after the source column correction
  - `raw_afs_documents` contains `25584` rows after the live import
  - sample imported rows include both file-backed documents and metadata-only rows
  ...
  - latest `import_products` run finished `success` with `imported_records = 47102`
